<template>
  <div class="verified-signup">
    ...
    <gr-top-hero class="verified-signup-top" title="Complete Signup" subtitle="One more final step to obtain your account">
    </gr-top-hero>
    ...
    <div class="columns is-mobile">
      <div class="column is-half-desktop is-10-mobile is-10-tablet is-offset-1-mobile
        ...
        <el-form :model="form" :rules="rules" ref="form" label-width="120px" label-position="top" class="demo-ruleForm">
          ...
          <el-form-item class="location-select" v-for="(branch, index) in form.branches" :key="index" :label="'Branch '+(index+1)" prop="branches">
            <el-input placeholder="Please Enter Address here" v-model="branch.address">
              <el-select v-model="branch.location" placeholder="Location" slot="prepend">
                <el-option v-for="location in locations" :key="location.value" :label="location.label" :value="location.value">
                </el-option>
              </el-select>
              <el-button @click="removeBranch(index)" slot="append">Delete Branch</el-button>
            </el-input>
            <el-button style="margin-top: 0.5em;" v-show="index === form.branches.length-1" @click="addBranch">New Branch</el-button>
          </el-form-item>
